...
//-----------------------------------------------------------------------------
Adxl345Controller::Adxl345Controller(const I2cInterface& i2c, uint16_t address)
   : i2c_( i2c )
   , initialized_( false )
   , address_( address )
   , last_error_( 0 )
...
bool Adxl345Controller::read_acceleration_data(AccelerationData& data)
{
   ...
   if ( i2c_.acquire( address_ ) == I2cInterface::kSuccess )
   {
      ...
      std::vector<std::byte> buffer(kBufferSize, 0);

      uint32_t bytes_received = 0;
      if ( i2c_.read(ADXL345_DATAX0, &buffer[0], buffer.size()) == I2cInterface::kSuccess)
      {
